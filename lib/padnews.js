...
(function(){
  ...
  Padnews = (function(){
    ...
    Padnews.prototype.getByApi = function(err, data){
      this.get(err, null, data);
    };
    Padnews.prototype.get = function(err, res, body){
      var last, i$, ref$, len$, line, date, j$, i, entry, news, prev;
      ...
      for (i$ = 0, len$ = (ref$ = body.split(regex.splitter)).length; i$ < len$; ++i$) {
        ...
        if (date) {
          for (j$ = this.news.length - 1; j$ >= 0; --j$) {
            i = j$;
            entry = this.news[i];
            if (!entry.month && !entry.date) {
              entry.month = date[1];
              entry.date = date[2];
              entry.timestamp = moment(this.year + "-" + padZero(entry.month) + "-" + padZero(entry.date) + "T" + entry.time).unix();
            }
          }
          continue;
        }
      ...
      for (i$ = 0, len$ = (ref$ = this.news).length; i$ < len$; ++i$) {
        entry = ref$[i$];
        if (!isNaN(entry.timestamp) && !isNaN(prev != null ? prev.timestamp : void 8)) {
    ...
    Padnews.prototype.didUpdate = function(){
    ...
    Padnews.prototype.run = function(){
      if (!this.apiClient) {
        request("https://" + this.domain + "hackpad.tw/ep/pad/static/" + this.id, this.get);
      } else {
